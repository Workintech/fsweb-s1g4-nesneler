...
function MenuElemaniOlustur(isimYeni, fiyatYeni, kategoriYeni) {
  const yeniMenuElemanlari = {};
  yeniMenuElemanlari.isim = isimYeni;
  yeniMenuElemanlari.fiyat = fiyatYeni;
  yeniMenuElemanlari.kategori = kategoriYeni;
  return yeniMenuElemanlari;

  //const yeniMenuElemanlari={
  //isim= isimYeni,
  //fiyat= fiyatYeni,
  //kategori= kategoriYeni,} şeklinde de yapabiliriz.
}
...
console.log(MenuElemaniOlustur("Cola", 2, "İçecekler"));
...
const burger = {
  ...
  indirim: function (meslek) {
    if (
      ...
      //toLowerCase() ile argümandan geleni küçültüp bizim belirlediğimiz yazım şekline eşitledik.
...
// }
//ikinci metot:
const found = degerlendirmeler.filter((item) => item.isim === "Ahmet");
console.log(found[0].geribildirim);
...
*/
const found2 = degerlendirmeler.filter((item) => item.isim === "Reyna");
found2[0].geribildirim =
...
function DegerledirmeEkle(degerlendirmeler, isimYeni, puanYeni, bildirimYeni) {
  const newObj = {};
  newObj.isim = isimYeni;
  newObj.puan = puanYeni;
  newObj.geribildirim = bildirimYeni;
...
function AnahtardanDegerlendirmeAl(gelenDizi, gelenKey) {
  const text =
    gelenDizi[gelenKey]["isim"] +
    " isimli kişi " +
    gelenDizi[gelenKey]["puan"] +
    " puan verdi ve şunları yazdı: " +
    gelenDizi[gelenKey]["geribildirim"];
  return text;
}
...
function PuanaGoreDegerlendirmeAl(dizi, gelenPuan) {
  return dizi.filter((item) => Math.floor(item.puan) === gelenPuan);
}
...
function UzunDegerlendirmeleriAl(dizi, number) {
  let yeniDizi = dizi.filter(
    (element) => element.geribildirim.split(" ").length > number
  );
  return yeniDizi;
}
